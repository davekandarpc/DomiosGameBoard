...
export class GameBoard extends Component {
  ...
  checkWinner = (data) => {
    ...
    if (board != undefined && board.length > 0) {
      ...
      for(var i =0 ; data.users.length > i ;i++){
        ...
        usersData.push(user)
        console.log("Winner Check User Data FINAL 2 : "+JSON.stringify(usersData))
      }
      console.log("Winner Check User Data FINAL 2: "+JSON.stringify(usersData))
  ...
  getUserList = async () => {
    ...
    if (response.status === 200) {
      ...
      console.log('USER LIST Stright Game : ' + JSON.stringify(response.user[0].UserId));
      console.log('USER LIST Stright Game : ' + JSON.stringify(global.UserId));
      console.log('USER LIST Stright Game Domino_Array length 0 : ' + JSON.parse(response.user[0].Domino_Array).length);
      console.log('USER LIST Stright Game Domino_Array length 1 : ' + JSON.parse(response.user[1].Domino_Array).length);
      var newUsers = []
      var userList = []
      const newPData = [
        {
      ...
      for(var i = 0;response.user.length>i ;i++){
        ...
        console.log("NEW USER ADDDEDD DATAAAA user length" + response.user.length)
        console.log("NEW USER ADDDEDD DATAAAA user length IIII" + i +" USER ID : "+ global.UserId)
  ...
  reload = async () => {
    const {userList} = this.state;
    console.log('REloaddd CALLLL :  :' + global.UserId);
    for (var i = 1; userList.length > i; i++) {
      if (userList[i] == global.UserId) {
    ...
    return (
      <SafeAreaView style={{flex: 1}}>
        ...
        <ImageBackground
          ...
          <View style = {{position:'absolute',flexDirection:'row',top:0,justifyContent:'space-around',width:'100%',}}>
